<x-layout>
    <x-slot name="title">{{__('message.bluedart.heading_title')}}</x-slot>
    <x-slot name="breadcrumbs">{{ Breadcrumbs::render('bluedart.create') }}</x-slot>
    <x-slot name="page_header_title"><h1 class="page-header-title">{{__('message.bluedart.page_header_title')}}</h1></x-slot>
    ...
    <x-slot name="main">  
        ...
        <div class="row">
            ...
            <div class="card col-sm-8">
                <div class="card-body">
                    <form class="needs-validation" method="POST" action="{{ route(panelPrefix().'.bluedart.store') }}" id="bluedart-form" novalidate>
