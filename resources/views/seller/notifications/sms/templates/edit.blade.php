<x-layout>
    <x-slot name="title">{{__('message.template_create.edit_tittle')}}</x-slot>
    <x-slot name="breadcrumbs">Notifications</x-slot>
    <x-slot name="page_header_title">
        <h1 class="page-header-title">{{__('message.template_create.edit_page_tittle')}}</h1>
    </x-slot>
    ...
    <x-slot name="main">
        <div class="card">
            <div class="card-body">
                <form action="{{ route('sms_template_update', ['id' => $template->id]) }}" method="POST">
                    @csrf
                    @method('PUT')   
                    <div class="row">                
                        <div class="col-md-6">
                            <div class="mb-4">
                                ...
                                <input type="text" name="template_registration_id" class="form-control" value="{{ old('template_registration_id', $template->template_registration_id) }}" required>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="mb-4">
                                ...
                                <div class="tom-select-custom">
                                    <select name="order_status" class="js-select form-select" required autocomplete="off"
                                        ...
                                        @foreach($shipment_statuses as $shipment_status)
                                            <option value="{{ $shipment_status->code }}" 
                                                {{ old('order_status', $template->order_status) == $shipment_status->code ? 'selected' : '' }}>
                        ...
                        <div class="col-md-12">
                            <div class="mb-4">
                                ...
                                <textarea name="message_content" class="form-control" rows="4" required>{{ old('message_content', $template->message_content) }}</textarea>
                            </div>
                        </div>
                                ...
                                <label class="form-control">
                                    <span class="form-check">
                                        <input 
                                            ...
                                            name="status"
                                            id="status"
                                            {{ old('status', $template->status) == '1' ? 'checked' : '' }} 
                                            required>
                                ...
                                <label class="form-control">
                                    <span class="form-check">
                                        <input 
                                            ...
                                            name="status"
                                            id="status_inactive"
                                            {{ old('status', $template->status) == '0' ? 'checked' : '' }} 
                                            required>
