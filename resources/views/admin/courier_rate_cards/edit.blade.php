<x-layout>
    <x-slot name="title">Edit Courier Rate</x-slot>
    <x-slot name="breadcrumbs">
         RateCard Edit
    </x-slot>

    <x-slot name="page_header_title">
        <h1 class="page-header-title">Edit Courier Rate</h1>
    </x-slot>
    ...
    <div class="card">
        <div class="card-header">
            <h4 class="card-header-title">Edit Courier Rate Card </h4>
        </div>
        <div class="card-body">
            <form action="{{ route('manage_rate_card.update', $courierRateCard->id) }}" method="POST">
                @csrf
                @method('PUT')
                        ...
                        <select name="courier_id" class="form-control" required>
                            ...
                            @foreach($couriers as $courier)
                                <option value="{{ $courier->id }}"
                                    {{ old('courier_id', $courierRateCard->courier_id) == $courier->id ? 'selected' : '' }}>
                                    {{ $courier->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="zone_name" class="form-control" required>
                            ...
                            @foreach(['A','B','C','D','E'] as $zone)
                                <option value="{{ $zone }}"
                                    {{ old('zone_name', $courierRateCard->zone_name) == $zone ? 'selected' : '' }}>
                                    {{ $zone }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="weight_slab_kg" class="form-select" required>
                            ...
                            @foreach ($weight_slabs as $value => $label)
                                <option value="{{ $value }}"
                                    {{ old('weight_slab_kg',$courierRateCard->weight_slab_kg) == $value ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="row mb-3">                    
                    <div class="col-md-6">
                        ...
                        <input type="number" step="0.01" name="base_freight_forward"
                            class="form-control"
                            value="{{ old('base_freight_forward', $courierRateCard->base_freight_forward) }}" required>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Additional Freight (₹)</label>
                        <input type="number" step="0.01" name="additional_freight"
                            class="form-control"
                            value="{{ old('additional_freight', $courierRateCard->additional_freight) }}" required>
                    </div>
                </div>
                <div class="row mb-3">   
                    <div class="col-md-6">
                        ...
                        <input type="number" step="0.01" name="rto_freight"
                            class="form-control"
                            value="{{ old('rto_freight', $courierRateCard->rto_freight) }}" required>
                    </div>
                     <div class="col-md-6">
                        <label class="form-label">COD Charge (₹)</label>
                        <input type="number" step="0.01" name="cod_charge"
                            class="form-control"
                            value="{{ old('cod_charge', $courierRateCard->cod_charge) }}" required>
                    </div>
                </div>
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="number" step="0.01" name="cod_percentage"
                            class="form-control"
                            value="{{ old('cod_percentage', $courierRateCard->cod_percentage) }}" required>
                    </div>
                    <div class="col-md-6">
                        ...
                        <select name="delivery_sla" class="form-select" required>
                            ...
                            @foreach ($slas as $value => $label)
                                <option value="{{ $value }}"
                                    {{ old('delivery_sla',$courierRateCard->delivery_sla) == $value ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio"
                                ...
                                {{ old('cod_allowed', $courierRateCard->cod_allowed) ? 'checked' : '' }}>
                            <label class="form-check-label" for="cod_yes">Yes</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio"
                                name="cod_allowed" id="cod_no" value="0"
                                {{ old('cod_allowed', $courierRateCard->cod_allowed) == 0 ? 'checked' : '' }}>
                            <label class="form-check-label" for="cod_no">No</label>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Sorting</label>
                        <input type="numeric" name="sort_order"  class="form-control" value="{{ old('sort_order', $courierRateCard->sort_order) }}" required>
                    </div>
                </div>
