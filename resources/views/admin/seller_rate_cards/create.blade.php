<x-layout>
    <x-slot name="title">Add Seller Rate</x-slot>
    <x-slot name="breadcrumbs"> RateCard Create </x-slot>

    <x-slot name="page_header_title">
        <h1 class="page-header-title">Add Seller Rate</h1>
    </x-slot>
    ...
    <div class="card">
        <div class="card-header">
            <h4 class="card-header-title">Create New Seller Rate Card </h4>
        </div>
        <div class="card-body">

            <form action="{{ route('manage_seller_rate_card.store') }}" method="POST">
                @csrf
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <select name="company_id" id="sellerSelect" class="form-control" required>
                            ...
                            @foreach ($sellers as $seller)
                                <option value="{{ $seller->id }}"
                                    {{ old('company_id') == $seller->id ? 'selected' : '' }}>
