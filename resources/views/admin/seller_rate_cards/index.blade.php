<x-layout>
    <x-slot name="title">Seller Rate Cards</x-slot>
    <x-slot name="breadcrumbs">
       RateCard List
    </x-slot>

    <x-slot name="page_header_title">
        <h1 class="page-header-title">Seller Rate Cards</h1>
    </x-slot>
     <x-slot name="headerbuttons">   
        ...
        @if(Request::segment(1) =='admin')    
            <div class="col-sm-auto">
                <a class="btn btn-primary btn-sm" href="{{ route('manage_seller_rate_card.create') }}"><i class="bi bi-plus-circle me-1"></i>Add New Rate</a>
                <a class="btn btn-primary btn-sm" href="{{ route('manage_seller_rate_card.import.form') }}"><i class="bi bi-plus-circle me-1"></i>Import CSV</a>
            </div>
        @endif
            ...
            <div class="card-header">
                ...
                <div class="row">
                  <div class="col-sm-12 mb-3">
                     ...
                     <select name="seller_company_id" id="sellerSelect" class="form-control">
                        <option value="">Select Seller</option>
                        @foreach ($sellers as $seller)
                              <option value="{{ $seller->id }}"
                                {{ (request('seller_company_id') == $seller->id || $seller_company_id == $seller->id) ? 'selected' : '' }}>
                        ...
                        @forelse($rateCards as $rateCard)
                            <tr>
                                ...
                                <!-- <td style="width:152px;">
                                    <a href="{{ route('manage_seller_rate_card.edit', $rateCard->id) }}"
                                    class="btn btn-sm btn-warning">
                                        Edit
                                    </a>

                                    <form action="{{ route('manage_seller_rate_card.destroy', $rateCard->id) }}"
                                        method="POST"
