...
class BluedartController extends Controller
{
    ...
    {
       return view('seller.couriers.settings.bluedart.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        $validatedData = $request->validate([
            ...
            'tracking_key' => 'required|string|max:255',
            'client_id' => 'required|string|max:255',
            'client_secret' => 'required|string|max:255',
            'origin_area' => 'required|string|max:10',
    ...
    public function edit(string $id)
    {
        ...
        $companyId = session('company_id');
        try {
            $bluedart = CourierSetting::where(['courier_id' => $id, 'company_id' => $companyId])->firstOrFail();
            $courier_details = json_decode($bluedart->courier_details,true);            
            $fields = [
                'login_id',
                'licence_key',
                'tracking_key',
            ...
            foreach ($fields as $field) {
                $bluedart->{$field} = $courier_details[$field] ?? '';
            }
            return view('seller.couriers.settings.bluedart.edit', compact('bluedart'));
    ...
    {   
        ...
        try{      
            ...
            $bluedart->update($post_data);

            // Update the associated Courier
            $courier = Courier::where(['id' => $bluedart->courier_id, 'company_id' => $companyId])->firstOrFail();
            $courier->update([
               // 'name' => $validatedData['courier_title'],
                'status' => $validatedData['status'],
            ]);
